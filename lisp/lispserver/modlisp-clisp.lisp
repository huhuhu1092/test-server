...
(defun make-socket-server (&key (name (format nil "socket server ~d "
  ...
  ;;	    domain port will be used.  (this will not work on Winows).
...
(defun myprocess-command (command)
    (handle-get-command command)
)
(defun process-apache-command (command)
  (let* ((url-content (cdr (assoc "url" command :test #'string=)))
         ...
         (html (cond ((equal url-content "/cchess/fixed") (debug-table command))
                     ((equal "/cchess/get" (car url-list)) (myprocess-command command))
		             (t (progn (format t "process-apache-command: ~a ~%" url-content) (handle-command url-content command))) 
               )
         )
        )
    (format t "~a ~%" url-list)
