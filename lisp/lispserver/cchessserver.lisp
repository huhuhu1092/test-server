...
(defun add-chess-session (red black)
  (let* (
    ...
    (if (not (session-exist? sym (www-world-sessionlist *world*)))
...
(defun handle-move (content)
  (let* (
    ...
    (format t "to user : ~a ~a~%" tousername directive)
    ...
    (let ((self-opponent (split-sequence #\Space content)))
      (let ((self (car self-opponent)) (opponent (cadr self-opponent)))
         (let ((sym (add-chess-session self opponent)))
           (send-message (make-www-message :from self :op 'start :content (list sym 'black)) opponent)
           (with-output-to-string (s)
             (format s "~a ~a" sym "red")
           )
         )
      )
    )
...
(defun message-to-string (message s)
  (format s "~a ~a ~a" (www-message-from message) (www-message-op message) (message-content-to-string (www-message-op message) (www-message-content message)))
)
;;first is user name
    ...
    (let ((message (split-sequence #\Space content)))
      (let* (
        ...
        (cond ((equal command "getallmessage") (progn
              ...
              ((equal command "getuser") (get-all-user-to-string))
        )
    )
   )
)
