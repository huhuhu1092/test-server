...
(defun session-name? (session-name session)
    (let ((sym (make-chess-sym (www-chess-session-red (caddr session)) (www-chess-session-black (caddr session))))
          (sym1 (make-chess-sym (www-chess-session-black (caddr session)) (www-chess-session-red (caddr session))))
         )
      (or (equal session-name sym) (equal session-name sym1))
    )
)
(defun get-session-by-name (session-name)
  (let (
        (session (mapcar #'(lambda (s) (if (session-name? session-name s) (caddr s))) (www-world-sessionlist *world*)))
       )
       (format t "get-session-byname : ~a~%" session)
       (first session)
  )
)
...
(defun handle-move (content)
  (let* (
        ...
        (fromusername (if session (get-chess-user-by-tag tag session) nil)) 
        (tousername (if session (get-chess-user-by-tag (opponent-chess-tag tag) session) nil))
       )
    (format t "handle-move: to user : ~a ~a ~a~%" tag tousername directive)
    (if (and fromusername tousername)
        (progn (send-message (make-www-message :from fromusername :op 'move :content directive) tousername)
                "send move ok")
        "send move error")
  )
)
...
(defun handle-getmessage (content)
    (let ((message (split-sequence #\Space content)))
       (cond ((equal "getuser" (car message)) (get-all-user-to-string))
        ...
        (t "error : input is not getmessage command")
   )
)
)
(defun remove-session-from-world (username)
  (let ((leftsession (remove-if #'(lambda (session) (or (equal (www-chess-session-red (caddr session)) username) (equal (www-chess-session-black (caddr session)) username))) (www-world-sessionlist *world*))))
...
(defun handle-command (url command)
  (cond ((equal url "/cchess/login") (handle-login (posted-content command)))
        ...
        (t "error : can not handle this command")
  )
)
...
(defun handle-get-move (content-list)
  (let* ( 
    ...
    (format t "~a~%" session-name)
    (format t "~a~%" tag)
    (format t "~a~%" directive)
...
(defun handle-get-logout (message)
    (let* ((wusername (car message))
          )
          (format t "logout: ~a~%" wusername)
...
(defun handle-get-command (command)
    (let* ((url-content (cdr (assoc "url" command :test #'string=)))
           ...
           (urlcommand (car command-list))
          )
          (format t "~a~%" url-content)
          (format t "~a~%" url-list)
          (cond ((equal urlcommand "login") (handle-get-login (cdr command-list)))
                ((equal urlcommand "logout") (handle-get-logout (cdr command-list)))
                ((equal urlcommand "move") (handle-get-move (cdr command-list)))
                ((equal urlcommand "getmessage") (handle-get-getmessage (cdr command-list)))
                (t "can not handle this urlcommand")
          )
    )
)
