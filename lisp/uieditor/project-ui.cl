...
(defun add-project (name path)
  (let ((canadd (not (or (contain-project-name? name) (contain-project-path? path))))
        )
      (print-project-num)
      (format t "name : ~a , path : ~a ~%" name path)
      (if canadd (add-project name path) (format t "the \"~a\" project or the path \"~a\" has been added~%" name path))
    )
  )
