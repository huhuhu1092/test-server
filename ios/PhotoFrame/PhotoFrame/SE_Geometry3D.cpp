...
SE_AABB::SE_AABB()
{}
SE_AABB SE_AABB::unionAABB(const SE_AABB& aabb)
{
    SE_Vector3f tmpMin = mMin, tmpMax = mMax;
    if(aabb.mMin.x < tmpMin.x)
        tmpMin.x = aabb.mMin.x;
    if(aabb.mMin.y < tmpMin.y)
        tmpMin.y = aabb.mMin.y;
    if(aabb.mMin.z < tmpMin.z)
        tmpMin.z = aabb.mMin.z;
    if(aabb.mMax.x > tmpMax.x)
        tmpMax.x = aabb.mMax.x;
    if(aabb.mMax.y > tmpMax.y)
        tmpMax.y = aabb.mMax.y;
    if(aabb.mMax.z > tmpMax.z)
        tmpMax.z = aabb.mMax.z;
    SE_AABB newAABB(tmpMin, tmpMax);
    return newAABB;
}
void SE_AABB::move(const SE_Vector3f& v)
{
    mMin = mMin + v;
    mMax = mMax + v;
}
