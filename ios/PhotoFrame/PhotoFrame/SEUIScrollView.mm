...
- (UIImage*) frameImage
{
    return mFrameImageView.image;
}
- (void) setFrameImage:(UIImage *)frameImage
{
    mFrameImageView.image = frameImage;
}
- (id) initWithFrame:(CGRect)frame
{
    ...
    if(self)
    {
        ...
        mImageView.backgroundColor = nil;//[UIColor blueColor];
        [self addSubview:mImageView];
        [mImageView release];
        
        mFrameImageView = [[UIImageView alloc] initWithFrame:bounds];
        mFrameImageView.clipsToBounds = YES;
        mFrameImageView.backgroundColor = nil;
        [self addSubview:mFrameImageView];
...
@end
/////
enum REMOVE_OP {LESS, GREAT, EQUAL};
...
@end
@interface SEUpdateImageEvent : SEUIScrollViewEvent {
@private
    int index;
}
@property (nonatomic, assign) int index;
...
- (void) removeImageFromView : (GarbageInfo*) gbInfo
{
    ...
    switch (gbInfo.op)
    {
        ...
        case EQUAL:
        {
            SEUIImageView* iv = [self getImageViewByIndex:index];
            iv.image = mDefaultImage;
        }
            break;
    }
        ...
        for(NSUInteger i = 0 ; i < count ; i++)
        {
            ...
            switch(event.type)
            {
                ...
                case SELOADPHOTO_EVENT:
                {
                    ...
                    break;
                case SEUPDATEIMAGE_EVENT:
                {
                    int index = ((SEUpdateImageEvent*)event).index;
                    [self removeFromPhotoDictUsingBlock:^BOOL(int photoIndex) {
                        return photoIndex == index;
                    }];
                    GarbageInfo* info = [[GarbageInfo alloc] init];
                    info.index = index;
                    info.op = EQUAL;
                    [self performSelectorOnMainThread:@selector(removeImageFromView:) withObject:info waitUntilDone:NO];
                    //[self removeImageFromView:info];
                }
                    break;
                default:
...
@synthesize mSelectedFrameImage;
    ...
    for(int i = 0 ; i < mRow && (!overflow) ; i++)
    {
        ...
        for(int k = 0 ; k < mColumn && (!overflow); k++)
        {
            if(j < totalPhotoCount)
            {
                ...
                if(mSelectedIndex != -1)
                {
                    imageView.frameImage = mSelectedFrameImage;
                }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if(mMultiTouchDetect.mTouchState == TOUCH2)
    {
        ...
        HitProperty hp = [self hitRect:p1];
        SEUIImageView* imageView1 = hp.imageView;
        hp = [self hitRect:p2];
        SEUIImageView* imageView2 = hp.imageView;
...
{
    if(index < 0 || index >= mParentView.subviews.count)
        return nil;
    return [mParentView.subviews objectAtIndex:index];
}
...
- (void) drawRect:(CGRect)rect
{
    ...
    {
        mBackgroundImage = [mResLoader getImage:@"ScrollViewBackground"];
    }
