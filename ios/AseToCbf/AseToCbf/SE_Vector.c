...
float SE_Vec3f_CrossScalar(const SE_Vector3f* leftV, const SE_Vector3f* rightV)
{
    double dot = SE_Vec3f_Dot(leftV, rightV);
    double len1 = SE_Vec3f_Length(leftV);
    double len2 = SE_Vec3f_Length(rightV);
    double theta = acos(dot / (len1 * len2));
    return (float)len1 * len2 * sin(theta);
}
