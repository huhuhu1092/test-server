...
#ifdef ARM_USE_VFP
#define DEBUG_DUMP_PATTERN "/tmp/misc/acc_dump/%d.c"
#else
...
class Compiler : public ErrorSink {
    ...
    protected:
        ...
        void SE_ASSERTImpl(bool test, int line) {
            if (!test) {
                error("code generator SE_ASSERTion failed at line %s:%d.", __FILE__, line);
                LOGI("code generator SE_ASSERTion failed at line %s:%d.", __FILE__, line);
                * (char*) 0 = 0;
            }
        }

        void setR0Type(Type* pType) {
            SE_ASSERT(pType != NULL);
    ...
    public:
        ...
        */
        ...
        virtual void over() {
            ...
            SE_ASSERT (r0ct == TY_INT  && tosct == TY_INT);
        ...
        virtual void storeR0ToTOS() {
            ...
            SE_ASSERT(pPointerType->tag == TY_POINTER);
    ...
    private:
        ...
        void setupFloatArgs() {
            ...
            if (tagTOS != TY_FLOAT) {
                SE_ASSERT(tagTOS == TY_INT);
                SE_ASSERT(tagR0 == TY_FLOAT);
#ifdef ARM_USE_VFP
    ...
    void SE_ASSERTImpl(bool isTrue, int line) {
        if (!isTrue) {
            LOGI("%d: SE_ASSERTion failed at line %s:%d.", mLineNumber, __FILE__, line);
            internalError();
        }
    }
    ...
    void unary() {
        ...
        for(;;) {
            if (tokl == 11) {
                ...
                SE_ASSERT(pFn->tag == TY_POINTER);
                SE_ASSERT(pFn->pHead->tag == TY_FUNC);
                pDecl = pFn->pHead;
                pGen->pushR0();
    ...
    Type* createType(TypeTag tag, Type* pHead, Type* pTail) {
        SE_ASSERT(tag >= TY_INT && tag <= TY_PARAM);
    ...
    Type* acceptStruct() {
        SE_ASSERT(tok == TOK_STRUCT || tok == TOK_UNION);
        ...
        if (structTag) {
            ...
            if (pStructInfo) {
                ...
                if (!fail) {
                     SE_ASSERT(pStructInfo->isStructTag);
