  ...
  for (i = 0; i < HASH_LAYERS; i ++) {
    ...
    if (HASH_POS_EQUAL(hsh, pos)) {
      ...
      if (hsh.ucBetaDepth > 0) {
		  ...
		  pipeOutputWrite(" lowerbound %d depth %d", hsh.svlBeta, hsh.ucBetaDepth);
      }
      if (hsh.ucAlphaDepth > 0) {
		  //printf(" upperbound %d depth %d", hsh.svlAlpha, hsh.ucAlphaDepth);
		  pipeOutputWrite(" upperbound %d depth %d", hsh.svlAlpha, hsh.ucAlphaDepth);
      }
      //printf("\n");
      //fflush(stdout);
	  pipeOutputWrite("\n");
