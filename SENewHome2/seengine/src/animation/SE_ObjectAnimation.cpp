...
{    
    ...
    if(background_for_clock)
    {
        ...
        float alpha = percent;
        background_for_clock->getMesh()->getSurface(0)->setAlpha(alpha - 0.3);
    }
}
...
{    
    ...
    {        
        float alpha = percent;
        background_for_clock->getMesh()->getSurface(0)->setAlpha(0.7 - alpha);
...
void SE_ObjectClockMoveAnimation::generateAlphaBackground()
{
	...
	c->mProgramDataID = FADEINOUT_SHADER;
	c->mRendererID = FADEINOUT_RENDERER;
	c->mLayerIndex = 1;
...
void SE_ObjectFadeInOutAnimation::onRun()
{
    ...
    {
        mSimObject->getMesh()->getSurface(i)->setProgramDataID(FADEINOUT_SHADER);
        mSimObject->getMesh()->getSurface(i)->setRendererID(FADEINOUT_RENDERER); 

        SE_BlendState *rs_blend = (SE_BlendState *)mSimObject->getSpatial()->getRenderState(SE_Spatial::BLENDSTATE);
...
void SE_ObjectFadeInOutAnimation::onEnd()
{
    ...
    for(int i = 0; i < surfaceNum; ++i)
    {
        ...
        SE_BlendState* blendState = (SE_BlendState *)mSimObject->getSpatial()->getRenderState(SE_Spatial::BLENDSTATE);
