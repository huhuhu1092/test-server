...
public:
	...
	void Write(SE_BufferOutput& output, SE_BufferOutput& outputScene, const char* shaderPath,const char *cbfName);
	void LoadEnd();
...
private:
    ...
    void processMatrix(ASE_GeometryObjectGroup* group);
    void processNewMatrix(ASE_GeometryObjectGroup* group, ASE_GeometryObject* parent);
    void findMaxFrameIndex(ASE_GeometryObjectGroup* group);
    void fillFrame(ASE_GeometryObjectGroup* group);
	...
	SE_Spatial *findGroupNode(SE_Spatial *root,const char *groupname,SE_CommonNode::NodeTypes type);
