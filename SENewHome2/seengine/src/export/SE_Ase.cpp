...
#else
    sCompressedTexture.setPixelType(pvrtexlib::OGL_PVRTC2);
#endif
...
void ASE_Loader::Write(SE_BufferOutput& output, SE_BufferOutput& outScene, const char* shaderPath, const char *cbfName)
{
...
//rewrite object name, use xxx.cbf\objname style
        ...
        tempIt++)
    {
		...
		std::string fullName = namecbf + "\\" + go->mGroupName + "\\"+ rawName;
    ...
    for(i = 0 ; i < materialNum ; i++)
    {
        ...
        if(srcm->numsubmaterials > 0)
        {
            ...
            if(srcm->bumpMaterialData)
            {
                md.md = *(srcm->bumpMaterialData);

                //push normal map material
                materialVector.push_back(md);
            }
    ...
    {
        std::string pathname = cbfName;
        size_t pos = pathname.rfind('\\');
        std::string namecbf = pathname.substr(pos+1, pathname.size());
    ...
    for(itMaterial = materialVector.begin() ; itMaterial != materialVector.end() ; itMaterial++)
    {
        ...
        if(texStr != "")
        {
            ...
            std::string tidstr = namecbf + "\\" + compressedName;
        ...
        itGeomObj++)
    {
        ...
        std::string gidstr = namecbf + "\\" + go->mGroupName + "\\"+ go->name;
    ...
    itGeomObj++)
    {
        ...
        std::string tcidstr = namecbf + "\\" + go->mGroupName + "\\"+ go->name;
    ...
    itGeomObj++)
    {
        ...
        vb.createVertexBuffer(mesh->numFaces,faces,vertexpos,tfaces,textureCoord);
...
///////////////////// write mesh ////////////////
    ...
    itGeomObj++)
    {
        ...
        std::string meshidstr = namecbf + "\\" + go->mGroupName +"\\" + go->name;
        ...
        SE_VertexBufferID vbID = geomTexCoordData[n].vbID;
        n++;
        geomID.write(output);

        //write vertex buffer id
        vbID.write(output);
...
///////////////////////////////add by liusong end///////////////////////////////////
        ...
        //find index in materialVector which materialref's

        for(int i = 0; i < materialref; ++i)
        {
            int materialCount = mSceneObject->mMats[i].numMaterials;

            if(materialCount == 0)
            {
                materialCount = 1;//because empty material has been in vector.
            }
            indexInVector += materialCount;
        }

        mdData = materialVector[indexInVector];
        
        //need rewrite
        if(mdData.subMaterialNum > 0 && 0)
        {
            int j;
            //for(j = 0 ; j < materialref - 1 ; j++)
            for(j = 0 ; j < materialref ; j++)//Do not j-1, need store how many material numbers before current geometry used
            {
        ...
        {
            std::string texname(mSceneObject->mMats[materialref].materialData.texName);
            //no sub materials
            if(!texname.empty())
            {
                texNum = 1;
        ...
        for(i = 0 ; i < texNum ; i++)
        {
            ...
            else
            {
                ...
                if(texStr != "")
                {
                    ...
                    output.writeInt(0);//texture unit type is TEXTURE0
                    texCoordID.write(output);
                    output.writeInt(mSceneObject->mMats[materialref].numMaterials);//image num use in the texture unit.If use normal,the number is 2,otherwise is 1.

                    for(int j = 0; j < mSceneObject->mMats[materialref].numMaterials; ++j)
                    {
                        mdData = materialVector[indexInVector + j];
    ...
    itGeomObj++)
    {
        ...
        if(!go->mGroupName.empty())
        {
            ...
            if(islodnode)
            {
                ...
                */
                lodNode = (SE_CommonNode*)findGroupNode(rootNode,lodName.c_str(),SE_CommonNode::LOD_NODE);
                if(!lodNode)
                {
                    //create a lod node
                    ...
                    lodNode->setBVType(SE_BoundingVolume::AABB);
                    lodNode->setGroupName(lodName.c_str());
                    lodNode->setNodeType(SE_CommonNode::LOD_NODE);
                }
            }
            ...
            if(!islodnode)
            {
                ...
                groupNode = (SE_CommonNode*)findGroupNode(rootNode,go->mGroupName.c_str(),SE_CommonNode::GROUP_NODE);
            }
            else
            {
                //the group is a child of the LOD node,must!
                groupNode = (SE_CommonNode*)findGroupNode(lodNode,go->mGroupName.c_str(),SE_CommonNode::GROUP_NODE);
            }
            ...
            if(!groupNode)
            {
                ...
                groupNode->setBVType(SE_BoundingVolume::AABB);
                groupNode->setGroupName(go->mGroupName.c_str());
                groupNode->setNodeType(SE_CommonNode::GROUP_NODE);
            }
...
void ASE_Loader::Write(const char* dataPath, const char* outFileName)
{
    ...
    Write(outBase, outScene, dataPath,outBaseFileName.c_str());
...
SE_Spatial *ASE_Loader::findGroupNode(SE_Spatial *root,const char *groupname,SE_CommonNode::NodeTypes type)
{
