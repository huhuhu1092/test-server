...
class SE_Vector3i;
    ...
    */
	class  BillboardSet: public SE_MeshSimObject
    {
    protected:
...
//		static RadixSort<ActiveBillboardList, Billboard*, float> mRadixSorter;
    ...
    public:
        ...
        */
        ...
        BillboardSet( const String& name, unsigned int poolSize, 
            bool externalDataSource, SE_Mesh* mesh);
...
//        virtual SortMode _getSortMode(void) const;
        ...
        */
        ...
        /** setTextureStacksAndSlices() will generate texture coordinate rects as if the 
          ...
          */
        virtual void setTextureStacksAndSlices( unsigned char stacks, unsigned char slices );
        ...
        /** getTextureCoords() returns the current texture coordinate rects in 
          ...
          @see
            BillboardSet::setTextureCoords()
          */
        virtual FloatRect const * getTextureCoords( uint16 * oNumCoords );
	...
	{
	protected:
		BillboardSet* createInstanceImpl( const String& name, const NameValuePairList* params);
	public:
		BillboardSetFactory() {}
		~BillboardSetFactory() {}
		...
		void destroyInstance( BillboardSet* obj);  
