...
//        mCastShadows = false;
	...
	BillboardSet::BillboardSet(
        const String& name,
        unsigned int poolSize,
        bool externalData, SE_Mesh* mesh) :
...
//        if (!billboardVisible(mCurrentCamera, bb)) return;
    ...
    {	
        this->getMesh()->getSurface(0)->upDateFaceVertex();
        this->getMesh()->getSurface(0)->upDateFaceTexVertex(0);
                			
        this->getMesh()->getGeometryData()->setParticleVertexArray(mVertex, mVertexDataIndex * 4);
        this->getMesh()->getGeometryData()->setParticleFaceArray(mIndexes, mVertexDataIndex * 2);
	    this->getMesh()->getSurface(0)->getTexture()->getTextureUnit(0)->getTextureCoordData()->setParticleTexVertexArray(mTexCoord, mVertexDataIndex * 4);
        this->getMesh()->getSurface(0)->getTexture()->getTextureUnit(0)->getTextureCoordData()->setParticleTexFaceArray(mIndexes, mVertexDataIndex * 2 );           
        this->getMesh()->getSurface(0)->setFacets(mFacet, mVertexDataIndex * 2); 
...
//        mMainBuf->unlock();
  ...
  //      */
		...
		if(mPointRendering)
		{
			mVertex = new SE_Vector3f[mPoolSize];
			mColor = new SE_Vector3f[mPoolSize];
		}
		else
		{
			mVertex = new SE_Vector3f[mPoolSize*4];
			mTexCoord = new SE_Vector2f[mPoolSize*4];	
			mColor = new SE_Vector3f[mPoolSize*4];
		}
  ...
  //      binding->setBinding(0, mMainBuf);
		...
		if(!mPointRendering)
		{
			mIndexes = new SE_Vector3i[mPoolSize * 2];
			mFacet = new int[mPoolSize * 2];
				...
				++bboard )
			{
				...
				mIndexes[idx].x = (idxOff); // + 0;, for clarity
				mIndexes[idx].y = (idxOff + 2);
				mIndexes[idx].z = (idxOff + 1);
				mIndexes[idx+1].x = (idxOff + 1);
				mIndexes[idx+1].y = (idxOff + 2);
				mIndexes[idx+1].z = (idxOff + 3);
...
//		return SceneManager::FX_TYPE_MASK;
        ...
        const Vector3* const offsets, const Billboard& bb)
    {
       ...
       // Root::getSingleton().convertColourValue(bb.mColour, &colour);
		...
		if (mPointRendering)
		{
			mVertex[mVertexDataIndex].x = bb.mPosition.x;
			mVertex[mVertexDataIndex].y = bb.mPosition.y;
			mVertex[mVertexDataIndex].z = bb.mPosition.z;
		//	// Single vertex per billboard, ignore offsets
		//	// position
  //          *mLockPtr++ = bb.mPosition.x;
  //          *mLockPtr++ = bb.mPosition.y;
  //          *mLockPtr++ = bb.mPosition.z;
			mColor[mVertexDataIndex].x= bb.mColour.r;
			mColor[mVertexDataIndex].y= bb.mColour.g;
			mColor[mVertexDataIndex].z= bb.mColour.b;
  //          // Colour
  ...
  //          // No texture coords in point rendering
		...
		else if (mAllDefaultRotation || bb.mRotation == Radian(0))
        {
            // Left-top
            // Positions
			mVertex[mVertexDataIndex*4].x = offsets[0].x + bb.mPosition.x;
			mVertex[mVertexDataIndex*4].y = offsets[0].y + bb.mPosition.y;
			mVertex[mVertexDataIndex*4].z = offsets[0].z + bb.mPosition.z;

           // *mLockPtr++ = offsets[0].x + bb.mPosition.x;
           // *mLockPtr++ = offsets[0].y + bb.mPosition.y;
           // *mLockPtr++ = offsets[0].z + bb.mPosition.z;
           // Colour
			mColor[mVertexDataIndex*4].x= bb.mColour.r;
			mColor[mVertexDataIndex*4].y= bb.mColour.g;
			mColor[mVertexDataIndex*4].z= bb.mColour.b;
  ...
  //          mLockPtr = static_cast<float*>(static_cast<void*>(pCol));
            // Texture coords
			mTexCoord[mVertexDataIndex*4].x = r.left;
			mTexCoord[mVertexDataIndex*4].y = r.top;
  //          *mLockPtr++ = r.left;
  //          *mLockPtr++ = r.top;

  //          // Right-top
  //          // Positions
			mVertex[mVertexDataIndex*4+1].x = offsets[1].x + bb.mPosition.x;
			mVertex[mVertexDataIndex*4+1].y = offsets[1].y + bb.mPosition.y;
			mVertex[mVertexDataIndex*4+1].z = offsets[1].z + bb.mPosition.z;

           // *mLockPtr++ = offsets[1].x + bb.mPosition.x;
          //  *mLockPtr++ = offsets[1].y + bb.mPosition.y;
           // *mLockPtr++ = offsets[1].z + bb.mPosition.z;
            // Colour
			mColor[mVertexDataIndex*4+1].x= bb.mColour.r;
			mColor[mVertexDataIndex*4+1].y= bb.mColour.g;
			mColor[mVertexDataIndex*4+1].z= bb.mColour.b;
  //          // Convert float* to RGBA*
  //          pCol = static_cast<RGBA*>(static_cast<void*>(mLockPtr));
  //          *pCol++ = colour;
  //          // Update lock pointer
  //          mLockPtr = static_cast<float*>(static_cast<void*>(pCol));
            // Texture coords
			mTexCoord[mVertexDataIndex*4+1].x = r.right;
			mTexCoord[mVertexDataIndex*4+1].y = r.top;
  //          *mLockPtr++ = r.right;
  //          *mLockPtr++ = r.top;

  //          // Left-bottom
  //          // Positions
			mVertex[mVertexDataIndex*4+2].x = offsets[2].x + bb.mPosition.x;
			mVertex[mVertexDataIndex*4+2].y = offsets[2].y + bb.mPosition.y;
			mVertex[mVertexDataIndex*4+2].z = offsets[2].z + bb.mPosition.z;
         //   *mLockPtr++ = offsets[2].x + bb.mPosition.x;
         //   *mLockPtr++ = offsets[2].y + bb.mPosition.y;
          //  *mLockPtr++ = offsets[2].z + bb.mPosition.z;
  //          // Colour
			mColor[mVertexDataIndex*4+2].x= bb.mColour.r;
			mColor[mVertexDataIndex*4+2].y= bb.mColour.g;
			mColor[mVertexDataIndex*4+2].z= bb.mColour.b;
  //          // Convert float* to RGBA*
  //          pCol = static_cast<RGBA*>(static_cast<void*>(mLockPtr));
  //          *pCol++ = colour;
  //          // Update lock pointer
  //          mLockPtr = static_cast<float*>(static_cast<void*>(pCol));
  //          // Texture coords
			mTexCoord[mVertexDataIndex*4+2].x = r.left;
			mTexCoord[mVertexDataIndex*4+2].y = r.bottom;
  //          *mLockPtr++ = r.left;
  //          *mLockPtr++ = r.bottom;

  //          // Right-bottom
  //          // Positions
			mVertex[mVertexDataIndex*4+3].x = offsets[3].x + bb.mPosition.x;
			mVertex[mVertexDataIndex*4+3].y = offsets[3].y + bb.mPosition.y;
			mVertex[mVertexDataIndex*4+3].z = offsets[3].z + bb.mPosition.z;
        //    *mLockPtr++ = offsets[3].x + bb.mPosition.x;
        //    *mLockPtr++ = offsets[3].y + bb.mPosition.y;
         //   *mLockPtr++ = offsets[3].z + bb.mPosition.z;
  //          // Colour
			mColor[mVertexDataIndex*4+3].x= bb.mColour.r;
			mColor[mVertexDataIndex*4+3].y= bb.mColour.g;
			mColor[mVertexDataIndex*4+3].z= bb.mColour.b;
  //          // Convert float* to RGBA*
  //          pCol = static_cast<RGBA*>(static_cast<void*>(mLockPtr));
  //          *pCol++ = colour;
  //          // Update lock pointer
  //          mLockPtr = static_cast<float*>(static_cast<void*>(pCol));
  //          // Texture coords
			mTexCoord[mVertexDataIndex*4+3].x = r.right;
			mTexCoord[mVertexDataIndex*4+3].y = r.bottom;
  //          *mLockPtr++ = r.right;
  //          *mLockPtr++ = r.bottom;
        }
        else if (mRotationType == BBR_VERTEX)
        {
			...
			mVertex[mVertexDataIndex*4].x = pt.x + bb.mPosition.x;
			mVertex[mVertexDataIndex*4].y = pt.y + bb.mPosition.y;
			mVertex[mVertexDataIndex*4].z = pt.z + bb.mPosition.z;
  ...
  //          *mLockPtr++ = r.top;
			...
			mVertex[mVertexDataIndex*4+1].x = pt.x + bb.mPosition.x;
			mVertex[mVertexDataIndex*4+1].y = pt.y + bb.mPosition.y;
			mVertex[mVertexDataIndex*4+1].z = pt.z + bb.mPosition.z;
  //          *mLockPtr++ = pt.x + bb.mPosition.x;
  //          *mLockPtr++ = pt.y + bb.mPosition.y;
  //          *mLockPtr++ = pt.z + bb.mPosition.z;
  //          // Colour
  ...
  //          *mLockPtr++ = r.top;
			...
			mVertex[mVertexDataIndex*4+2].x = pt.x + bb.mPosition.x;
			mVertex[mVertexDataIndex*4+2].y = pt.y + bb.mPosition.y;
			mVertex[mVertexDataIndex*4+2].z = pt.z + bb.mPosition.z;
  //          *mLockPtr++ = pt.x + bb.mPosition.x;
  //          *mLockPtr++ = pt.y + bb.mPosition.y;
  //          *mLockPtr++ = pt.z + bb.mPosition.z;
  //          // Colour
  ...
  //          *mLockPtr++ = r.bottom;
			...
			mVertex[mVertexDataIndex*4+3].x = pt.x + bb.mPosition.x;
			mVertex[mVertexDataIndex*4+3].y = pt.y + bb.mPosition.y;
			mVertex[mVertexDataIndex*4+3].z = pt.z + bb.mPosition.z;
  //          *mLockPtr++ = pt.x + bb.mPosition.x;
  //          *mLockPtr++ = pt.y + bb.mPosition.y;
  //          *mLockPtr++ = pt.z + bb.mPosition.z;
            // Colour
  ...
  //          mLockPtr = static_cast<float*>(static_cast<void*>(pCol));
  //          // Texture coords
			mTexCoord[mVertexDataIndex*4+3].x = r.left;
			mTexCoord[mVertexDataIndex*4+3].y = r.bottom;
  //          *mLockPtr++ = r.right;
  //          *mLockPtr++ = r.bottom;
        }
  ...
  //          mLockPtr = static_cast<float*>(static_cast<void*>(pCol));
            // Texture coords
			mTexCoord[mVertexDataIndex*4].x = mid_u - cos_rot_w + sin_rot_h;
			mTexCoord[mVertexDataIndex*4].y = mid_v - sin_rot_w - cos_rot_h;
  ...
  //          mLockPtr = static_cast<float*>(static_cast<void*>(pCol));
  //          // Texture coords
			mTexCoord[mVertexDataIndex*4+1].x = mid_u + cos_rot_w + sin_rot_h;
			mTexCoord[mVertexDataIndex*4+1].y = mid_v + sin_rot_w - cos_rot_h;
  //          *mLockPtr++ = mid_u + cos_rot_w + sin_rot_h;
  //          *mLockPtr++ = mid_v + sin_rot_w - cos_rot_h;
  ...
  //          mLockPtr = static_cast<float*>(static_cast<void*>(pCol));
  //          // Texture coords
			mTexCoord[mVertexDataIndex*4+2].x = mid_u - cos_rot_w - sin_rot_h;
			mTexCoord[mVertexDataIndex*4+2].y = mid_v - sin_rot_w + cos_rot_h;
  //          *mLockPtr++ = mid_u - cos_rot_w - sin_rot_h;
  //          *mLockPtr++ = mid_v - sin_rot_w + cos_rot_h;
  ...
  //          mLockPtr = static_cast<float*>(static_cast<void*>(pCol));
            // Texture coords
			mTexCoord[mVertexDataIndex*4+3].x = mid_u + cos_rot_w - sin_rot_h;
			mTexCoord[mVertexDataIndex*4+3].y = mid_v + sin_rot_w + cos_rot_h;
  //          *mLockPtr++ = mid_u + cos_rot_w - sin_rot_h;
  //          *mLockPtr++ = mid_v + sin_rot_w + cos_rot_h;
        }
		++mVertexDataIndex;
